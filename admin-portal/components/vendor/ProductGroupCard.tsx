...
import type { ProductGroup, ProductVariant } from '@/lib/utils/productGrouping';
import { getEmojiForCategory, getColorsForSize } from '@/lib/utils/productGrouping';
...
export default function ProductGroupCard({
  ...
  const colorsForSize = useMemo(() => {
    ...
    return getColorsForSize({ ...group, variants: availableVariants }, selectedSize);
  ...
  const priceDisplay =
    group.basePrice === group.maxPrice
      ...
      : `$${group.basePrice.toLocaleString('es-CO')} - $${group.maxPrice.toLocaleString('es-CO')}`;
    ...
    >
      ...
      {addedQuantity > 0 && (
        <div className="absolute -top-2 -right-2 bg-green-500 text-white text-xs font-bold px-2 py-0.5 rounded-full shadow-sm flex items-center gap-1 z-10">
          <CheckCircle className="w-3 h-3" />+{addedQuantity}
        </div>
      )}
      ...
      <div className="mb-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          {availableSizes.map((size) => {
            ...
            const isExcluded = availableVariants
              .filter((v) => v.size === size)
              .every((v) => excludeProductIds.includes(v.productId));
      ...
      {selectedVariant && selectedVariant.stock > 0 && quantity > selectedVariant.stock && (
        <div className="bg-orange-50 border border-orange-200 rounded-lg p-2 mb-3 text-sm">
          <p className="text-orange-700">
            Solo hay <strong>{selectedVariant.stock}</strong> unidades disponibles.
            {quantity - selectedVariant.stock > 0 && (
              <span> ({quantity - selectedVariant.stock} seria por encargo)</span>
            )}
          </p>
        </div>
      )}
          ...
          <div className="flex items-center border border-gray-300 rounded-lg">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                // If stock > 0, limit to available stock
                if (selectedVariant.stock > 0) {
                  setQuantity(Math.min(val, selectedVariant.stock));
                } else {
                  setQuantity(val);
                }
              }}
